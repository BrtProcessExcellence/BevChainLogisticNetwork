import { PROVINCE_NAME_MAP } from '../../config/constants.js';

/**
 * ==============================================================================
 * MAP CONTROLLER & VISUALIZATION ENGINE (ES MODULE)
 * ==============================================================================
 */

// 1. Global Map Instances & Layer Groups
export let dashMap = null,
  simMap = null,
  execMap = null;
let dashLayerGrp = null,
  simLayerGrp = null;
...
// ==============================================================================
function isDarkMode() {
  return typeof window.state !== 'undefined' && Boolean(window.state?.isDark);
}
...
export function updateMapTiles() {
  const currentTheme = (typeof window.state !== 'undefined' && window.state?.mapTheme) || 'google-hybrid';
  let tileUrl = '';
  let subdomains = ['mt0', 'mt1', 'mt2', 'mt3'];

  if (currentTheme === 'google-hybrid') tileUrl = 'https://{s}.google.com/vt/lyrs=y&x={x}&y={y}&z={z}';
  else if (currentTheme === 'google-satellite') tileUrl = 'https://{s}.google.com/vt/lyrs=s&x={x}&y={y}&z={z}';
  else if (isDarkMode()) {
...
function resolveDestinationCoords(rowOrDesc, provinceFallback = '') {
  ...
  if (typeof rowOrDesc === 'object') {
    ...
    if (!isNaN(lat) && !isNaN(lng) && isValidThailandCoord(lat, lng)) return [lat, lng];
  }
  const province =
    typeof rowOrDesc === 'object'
...
function resolveLocationCoords(locationKey) {
  ...
  if (window.shippingLocationLookup) {
    if (window.shippingLocationLookup[exactKey])
      result = [window.shippingLocationLookup[exactKey].lat, window.shippingLocationLookup[exactKey].lng];
    else if (window.shippingLocationLookup[cleanKey])
      result = [window.shippingLocationLookup[cleanKey].lat, window.shippingLocationLookup[cleanKey].lng];
  }
  if (!result && typeof window.originLocationMap !== 'undefined') {
    if (window.originLocationMap[exactKey])
      result = [window.originLocationMap[exactKey].lat, window.originLocationMap[exactKey].lng];
    else if (window.originLocationMap[cleanKey])
      result = [window.originLocationMap[cleanKey].lat, window.originLocationMap[cleanKey].lng];
  }
  if (!result && typeof window.provinceLocationMap !== 'undefined') {
    if (window.provinceLocationMap[exactKey])
      result = [window.provinceLocationMap[exactKey].lat, window.provinceLocationMap[exactKey].lng];
    else if (window.provinceLocationMap[cleanKey])
      result = [window.provinceLocationMap[cleanKey].lat, window.provinceLocationMap[cleanKey].lng];
    else {
      const foundProv = Object.keys(window.provinceLocationMap).find(
        (k) => cleanAllSpaces(k).includes(cleanKey) || cleanKey.includes(cleanAllSpaces(k))
      );
      if (foundProv) result = [window.provinceLocationMap[foundProv].lat, window.provinceLocationMap[foundProv].lng];
    }
  }
...
export async function renderExecRouteHeatmap(data) {
  if (!execMap) return;
  ...
  data.forEach((row) => {
    ...
    const trips = parseSafeNum(row['AVG Trip/Week'] || row.avg_trip_week || row.avg_trips, 0);
    ...
    if (!provMap[cleanProv])
      provMap[cleanProv] = { totalRoutes: 0, availRoutes: 0, totalTrips: 0, availTrips: 0, displayName: rawProv };
  ...
  try {
    ...
    if (execGeoJsonLayer && execMap.hasLayer(execGeoJsonLayer)) execMap.removeLayer(execGeoJsonLayer);
    execMap.eachLayer((layer) => {
      if (layer instanceof L.GeoJSON) execMap.removeLayer(layer);
    });
    ...
    execGeoJsonLayer = L.geoJSON(geoData, {
      ...
      onEachFeature: (feature, layer) => {
        ...
        const rawGeoName = feature.properties?.name || feature.properties?.name_th || '';
        const displayTitle = stat?.displayName || getThaiProvinceName(rawGeoName);

        layer._baseColor = getExecChoroplethColor(hasData ? availPct : null);
        ...
        layer.bindTooltip(
          ...
          <div class="px-2.5 py-1.5 min-w-[200px] font-sans">
            ...
            ${
              hasData
                ? `
              <div class="flex justify-between items-center mb-1"><span class="text-slate-500 dark:text-slate-400 text-[11px]">Available Backhaul:</span><strong class="text-emerald-600 dark:text-emerald-400 font-extrabold text-xs">${availRoutes.toLocaleString()} routes (${availPct.toFixed(2)}%)</strong></div>
              <div class="flex justify-between items-center text-[10px] text-slate-400 pt-1 border-t border-slate-100 dark:border-slate-800"><span>Total Registered:</span><span>${totalRoutes.toLocaleString()} routes (~${Math.round(totalTrips).toLocaleString()} trips/wk)</span></div>
              <div class="flex justify-between items-center text-[10px] text-emerald-600 dark:text-emerald-400 font-medium"><span>Available Volume:</span><span>~${Math.round(availTrips).toLocaleString()} trips/wk</span></div>
            `
        ...
        layer.on({
          ...
          mouseout: (e) => {
            ...
            if (currentSelectedZone) {
              l.setStyle(
                l._isZoneMatch
                  ? { fillColor: l._baseColor, weight: 2.5, color: '#f97316', opacity: 1.0, fillOpacity: 0.85 }
                  : { fillColor: '#94a3b8', weight: 0.8, color: '#cbd5e1', opacity: 0.35, fillOpacity: 0.22 }
              );
            } else {
              l.setStyle({
...
export function highlightRegionOnExecMap(targetZone, sourceData = null) {
  ...
  const data = sourceData || window.globalRouteSheetData || [];
  ...
  execGeoJsonLayer.eachLayer((layer) => {
    ...
    const isMatch =
      provincesInZone.has(cleanGeo) ||
      provincesInZone.has(cleanThai) ||
      provList.some((p) => cleanThai.includes(p) || p.includes(cleanThai));
    layer._isZoneMatch = isMatch;

    if (isMatch) {
      layer.setStyle({ fillColor: layer._baseColor, weight: 2.5, color: '#f97316', opacity: 1.0, fillOpacity: 0.85 });
      layer.bringToFront();
      matchedBounds.push(layer.getBounds());
    } else {
      layer.setStyle({ fillColor: '#94a3b8', weight: 0.8, color: '#cbd5e1', opacity: 0.35, fillOpacity: 0.22 });
    }
  });
...
export function resetExecMapHighlight() {
  if (!execMap || !execGeoJsonLayer) return;
  currentSelectedZone = null;
  execGeoJsonLayer.eachLayer((layer) => {
...
// ==============================================================================
// 5. DASHBOARD ROUTE & HEATMAP VISUALIZATION
// ==============================================================================
export function updateMapDisplay(filteredData) {
...
function renderDashboardLayersByMode(filteredData = []) {
  if (!dashMap) return;
  const displayMode = (typeof window.state !== 'undefined' && window.state?.activeFilters?.displayMode) || 'routes';
  const metric = (typeof window.state !== 'undefined' && window.state?.activeFilters?.heatMetric) || 'volume';
  const theme = (typeof window.state !== 'undefined' && window.state?.activeFilters?.heatTheme) || 'thermal';
  const radius = (typeof window.state !== 'undefined' && window.state?.activeFilters?.heatRadius) || 35;
...
function drawDashboardRoutes(filteredData = []) {
  ...
  if (dashLayerGrp) dashLayerGrp.clearLayers();
  else dashLayerGrp = L.layerGroup().addTo(dashMap);
  ...
  if (mapContainer && (mapContainer.offsetWidth === 0 || mapContainer.offsetHeight === 0)) dashMap.invalidateSize();
  ...
  Object.values(routeMap).forEach((route, index) => {
    ...
    const destCoords = resolveDestinationCoords(
      route.rawRow || { 'Description(Ship-To (Outbound))': route.to, จังหวัด: route.province }
    );
    ...
    const handleRouteClick = (e) => {
      ...
      if (typeof window.focusTableRowByMapKey === 'function') window.focusTableRowByMapKey(route.key);
    };
...
export function highlightMapRoute(targetKey) {
  ...
  Object.keys(routePolylineMap).forEach((key) => {
    ...
    if (key === targetKey) {
      ...
      if (item.destDot) item.destDot.setStyle({ opacity: 0.15, fillOpacity: 0.15 });
    }
  });
}

export function resetMapRouteStyles() {
  if (!routePolylineMap) return;
  currentHighlightedKey = null;
  currentHighlightedOrigin = null;
  document
    .querySelectorAll('[id^="dc-pin-"]')
    .forEach((el) => el.classList.remove('ring-4', 'ring-orange-400', 'scale-110'));
  ...
  const currentMode = typeof window.state !== 'undefined' ? window.state?.activeFilters?.displayMode : 'routes';
  if (currentMode === 'heatmap' || currentMode === 'hybrid') {
    const activeData =
      typeof window.currentFilteredData !== 'undefined' && window.currentFilteredData.length > 0
        ? window.currentFilteredData
        : window.globalRouteSheetData || [];
    renderHeatmap(
      activeData,
      window.state?.activeFilters?.heatMetric || 'volume',
      window.state?.activeFilters?.heatTheme || 'thermal',
      window.state?.activeFilters?.heatRadius || 35
    );
  }
}

export function renderUniqueDCPins(filteredData, targetMap = dashMap, targetLayerGrp = dashLayerGrp) {
  if (!targetMap || !targetLayerGrp || !filteredData || filteredData.length === 0) return;
  const uniqueOrigins = [
    ...new Set(filteredData.map((row) => String(row['ต้นทาง'] || row.origin || '').trim()).filter(Boolean))
  ];
  ...
  uniqueOrigins.forEach((originName) => {
    ...
    const customHtml = `<div id="dc-pin-${cleanOrigin}" class="hub-badge hub-badge-plant shadow-md cursor-pointer transition-all duration-300 ${isFocused ? 'ring-4 ring-orange-400 scale-110' : 'hover:scale-105'}"><span class="node-pulse-dot bg-orange-500"></span><span class="font-bold font-sans">${originName}</span></div>`;
...
function highlightRoutesByOrigin(originName, originCoords) {
  ...
  Object.keys(routePolylineMap).forEach((key) => {
    const item = routePolylineMap[key];
    if (key.startsWith(`${cleanTargetOrigin}__`)) {
      ...
      if (item.destDot) item.destDot.setStyle({ opacity: 0.05, fillOpacity: 0.05 });
    }
  });
  ...
  if (activePin) activePin.classList.add('ring-4', 'ring-orange-400', 'scale-110');
  if (dashMap && matchedBounds.length > 0)
    dashMap.fitBounds(L.latLngBounds(matchedBounds), { padding: [60, 60], maxZoom: 9 });
  if (typeof window.filterTableByOrigin === 'function') window.filterTableByOrigin(originName);
}

export function renderHeatmap(filteredData, metric, themeKey, radius) {
  if (currentHeatLayer && dashMap) {
    dashMap.removeLayer(currentHeatLayer);
    currentHeatLayer = null;
  }
  ...
  let targetData = currentHighlightedOrigin
    ? filteredData.filter((item) => {
        const orig = cleanAllSpaces(item['ต้นทาง'] || item.origin || '');
        return orig.includes(currentHighlightedOrigin);
      })
    : filteredData;
  ...
  targetData.forEach((item) => {
    const originCoords = resolveLocationCoords(item['ต้นทาง'] || item.origin);
    const destCoords = resolveDestinationCoords(item, item['จังหวัด'] || item.province);
    let w = 1.0;
    if (metric === 'volume') {
      const vol = parseSafeNum(item['AVG Trip/Week'] || item.avg_trip_week, 0);
      w = vol > 0 ? vol : 1;
    ...
    if (originCoords && isValidThailandCoord(originCoords[0], originCoords[1]))
      nodeWeightMap[`${originCoords[0]},${originCoords[1]}`] =
        (nodeWeightMap[`${originCoords[0]},${originCoords[1]}`] || 0) + w;
    if (destCoords && isValidThailandCoord(destCoords[0], destCoords[1]))
      nodeWeightMap[`${destCoords[0]},${destCoords[1]}`] =
        (nodeWeightMap[`${destCoords[0]},${destCoords[1]}`] || 0) + w;
  });
  ...
  Object.entries(nodeWeightMap).forEach(([coordStr, totalW]) => {
    ...
    for (let i = 0; i < 3; i++)
      heatPoints.push([lat + (Math.random() - 0.5) * 0.018, lon + (Math.random() - 0.5) * 0.018, normW * 0.8]);
  });

  if (heatPoints.length > 0 && typeof L.heatLayer === 'function') {
    const themes = { thermal: { 0.2: '#3b82f6', 0.4: '#10b981', 0.6: '#eab308', 0.8: '#f97316', 1.0: '#ef4444' } };
    currentHeatLayer = L.heatLayer(heatPoints, {
      radius: parseInt(radius) || 45,
      blur: 20,
      maxZoom: 18,
      max: 0.6,
      minOpacity: 0.35,
      gradient: themes[themeKey] || themes.thermal,
      pane: 'heatPane'
    }).addTo(dashMap);
  }
}

export async function drawAllSheetRoutesOnSimMap(
  subconData,
  originName,
  provinceName,
  customerInfo = {},
  ...
  const startLat = originInfo?.lat || (resolveLocationCoords(originName) || [13.75, 100.5])[0];
  const startLng = originInfo?.lng || (resolveLocationCoords(originName) || [13.75, 100.5])[1];
  const endLat = customerInfo?.lat || (resolveDestinationCoords({ จังหวัด: provinceName }) || [14.0, 100.6])[0];
  const endLng = customerInfo?.lng || (resolveDestinationCoords({ จังหวัด: provinceName }) || [14.0, 100.6])[1];

  L.marker([startLat, startLng], {
    icon: L.divIcon({
      className: 'custom-hub-marker',
      html: `<div class="hub-badge hub-badge-plant"><span class="node-pulse-dot bg-orange-500"></span><span>${originName}</span></div>`
    })
  }).addTo(simLayerGrp);
  L.marker([endLat, endLng], {
    icon: L.divIcon({
      className: 'custom-hub-marker',
      html: `<div class="hub-badge hub-badge-shipto"><span class="node-pulse-dot bg-blue-500"></span><span>${customerInfo.name || provinceName}</span></div>`
    })
  }).addTo(simLayerGrp);

  const curvePoints = getCurvePoints(startLat, startLng, endLat, endLng, 0.05);
  subconData.forEach((item, index) => {
    const subconId = String(item['ID'] || item.id || index);
    const layerGroup = L.layerGroup();
    const mainLine = L.polyline(curvePoints, { color: '#10b981', weight: 3.5, opacity: 0.95 });
